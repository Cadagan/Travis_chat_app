    ...
    chatImgUpload( req, res, ( error ) => {
        ...
        if( error ){
            ...
            if( req.file === undefined ){
                ...
                insertImageToDatabase(imageLocation, req.params.roomid)
                    .then(result => {
...
router.get('/:roomid/image', function(req, res, next){
   ...
   client.query(query, (err, queryRes)=>{
      if(err){
          ...
          const data = {roomImage: "https://www.elcomercio.com/files/article_main/uploads/2016/10/28/5813ffd918991.jpeg"};
...
router.get('/', function(req, res, next) {
    ...
    const query = {
        ...
        name: 'fetch-user',
        text: 'SELECT name,id FROM rooms'
    };
    ...
    client.query(query, (err, queryRes) => {
        if (err) {
            ...
            queryRes.rows.forEach(room=>{
              rooms.push({"roomName":room.name, "roomId": room.id})
            })
        }
...
router.post('/new', function(req, res, next) {
    ...
    console.log(`Inserting ${roomName} into the database`);
    res.status(200);
    if(roomName!==undefined){
        insertToDatabase(roomName).then(r => {
...
async function insertToDatabase(roomName){
    // we don't need to dispose of the client (it will be undefined)
    try {
        await client.query('BEGIN');
        const queryText = "INSERT INTO rooms(name) VALUES ($1)";
        const res = await client.query(queryText, [roomName]);
