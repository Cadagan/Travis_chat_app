...
>>>>>>> 33f746efcd48fa419aa34820e1f0a38632a9c00d
    }
    console.log('Connected to socket.io endpoint!');
    this.mesRef = React.createRef();
  }
...
=======
  singleFileUploadHandler = event => {
    ...
    if (this.state.selectedFile) {
      ...
      axios
        .post(
          `${BACKEND_HOST}/rooms/${this.sessionData.roomId}/chat-img-upload`,
          data,
          {
            headers: {
              accept: 'application/json',
        ...
        .then(response => {
          if (200 === response.status) {
            ...
            if (response.data.error) {
              ...
              let fileName = response.data;
              console.log(
                `${fileName}, Este es el link a guardar en atributo imagen: ${fileName.location}, `,
              );
              this.setState({roomImage: fileName.location});
              this.ocShowAlert('File Uploaded', '#3089cf');
            }
